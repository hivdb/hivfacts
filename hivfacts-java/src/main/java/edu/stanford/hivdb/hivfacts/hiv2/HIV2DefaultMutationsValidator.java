...
public class HIV2DefaultMutationsValidator implements MutationsValidator<HIV2> {
	...
	private List<ValidationResult> validateNoStopCodons(MutationSet<HIV2> mutations) {
		List<ValidationResult> validationResults = new ArrayList<>();
	...
	private List<ValidationResult> validateNoTooManyUnusualMutations(MutationSet<HIV2> mutations) {
		List<ValidationResult> validationResults = new ArrayList<>();
		MutationSet<HIV2> unusualMutations = mutations.getUnusualMutations();
		int numUnusual = unusualMutations.size();

		MutationSet<HIV2> unusualMutAtDRP = unusualMutations.getAtDRPMutations();
		...
		if (numUnusual > 1) {
			if (numUnusualAtDRP == 0) {
				...
				MutationSet<HIV2> additionalMuts = unusualMutations.subtractsBy(unusualMutAtDRP);
				validationResults.add(newValidationResult(
	...
	private List<ValidationResult> validateNotApobec(MutationSet<HIV2> mutations) {
		MutationSet<HIV2> apobecs = mutations.getApobecMutations();
		MutationSet<HIV2> apobecDRMs = mutations.getApobecDRMs();
